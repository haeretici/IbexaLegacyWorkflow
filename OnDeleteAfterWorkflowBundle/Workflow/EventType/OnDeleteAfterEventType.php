...
namespace Haeretici\OnDeleteAfterWorkflowBundle\Workflow\EventType;
...
class OnDeleteAfterEventType extends AbstractWorkflowEventType
{
    public const TYPE_STRING = 'event_haeretici_ondeleteafter';

    public function __construct(
        private readonly OnDeleteAfterEventTypeLogger $logger,
    ) {
        parent::__construct(
            self::TYPE_STRING,
            'On delete after',
    ...
    public function execute(WorkflowProcess $process, WorkflowEvent $event): int
    {
        ...
        $this->setInformation(sprintf(
            'Post-delete hook ran for content %d version %d',
